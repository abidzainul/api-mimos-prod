  ...
  public function injectSellingDetail_post(){
    ...
    if ($sellinid === null) {
     ...
     'createdon' => date('Y-m-d H:i:s'),
      'createdms' =>$this->udate('Y-m-d H:i:s.u T')
    );
     ...
     $dataupdate = array(
      ...
      'updatedms' =>$this->udate('Y-m-d H:i:s.u T')
    );
     //--- cek data 
...
public function injectPosmHead_post(){
  ...
  if ($trx === null) {
    ...
    if ($transac){
      ...
      $transacid = $transac[0]['id'];
      $arrstrsql = array();
      $strsql ="Update posm  set customerno ='".$customerno."',posmdate ='".$tglkunjungan."',userid ='".$userid."',regionid ='".$regionid."',salesofficeid ='".$salesofficeid."',salesgroupid ='".$salesgroupid."',salesdistrictid ='".$salesdistrictid."',cycle ='".$cycle."',week ='".$week."',year ='".$year."',updatedby ='".$userid."',updatedon = Now(),updatedms ='".$this->udate('Y-m-d H:i:s.u T')."' where id ='".$transacid."';" ;
      $arrstrsql[0]['strsql'] =$strsql;
...
public function injectStockHead_post(){
  ...
  if ($trx === null) {
    ...
    if($transac){
      ...
      $transacid=$transac[0]['id'];
      $arrstrsql = array();
      $strsql ="Update stock  set customerno ='".$customerno."',stockdate ='".$tglkunjungan."',userid ='".$userid."',regionid ='".$regionid."',salesofficeid ='".$salesofficeid."',salesgroupid ='".$salesgroupid."',salesdistrictid ='".$salesdistrictid."',cycle ='".$cycle."',week ='".$week."',year ='".$year."',updatedby ='".$userid."',updatedon = Now(), updatedms='".$this->udate('Y-m-d H:i:s.u T')."' where id ='".$transacid."';" ;
      $arrstrsql[0]['strsql'] =$strsql;
...
public function injectVisibilityHead_post(){
  ...
  if ($trx === null) {
    ...
    if($transac){
      ...
      $transacid=$transac[0]['id'];
      $arrstrsql = array();
      $strsql ="Update visibility  set customerno ='".$customerno."',visibilitydate ='".$tglkunjungan."',userid ='".$userid."',regionid ='".$regionid."',salesofficeid ='".$salesofficeid."',salesgroupid ='".$salesgroupid."',salesdistrictid ='".$salesdistrictid."',cycle ='".$cycle."',week ='".$week."',year ='".$year."',updatedby ='".$userid."',updatedon = Now(),updatedms ='".$this->udate('Y-m-d H:i:s.u T')."' where id ='".$transacid."';" ;
      $arrstrsql[0]['strsql'] =$strsql;
...
//========= end visibility
//---- visit pause
public function injectVisitPause_post(){
  $visitid = $this->post('trxid');
  $starttime = $this->post('starttime');
  $stoptime = $this->post('stoptime');
  $userid = $this->post('userid');

  if ($visitid === null) {
    $this->response([
      'status' => FALSE,
      'message' =>'Visit ID null',
      'data' => 'Visit ID Null'
    ], REST_Controller::HTTP_BAD_REQUEST);
  }else if($visitid ==""){
    $this->response([
      'status' => FALSE,
      'message' =>'Visit ID kosong',
      'data' => 'Visit ID Kosong'
    ], REST_Controller::HTTP_BAD_REQUEST);
  }else if($starttime ==""){
    $this->response([
      'status' => FALSE,
   ...
   $cekdata = $this->Model_InjectDB->cek_Data_VisitPause($visitid,$starttime);

     if ($cekdata){
        //---update data
        $transacid = $cekdata[0]['id'];
        $this->db->where('id', $transacid);
        $prosesupdate = $this->db->update('visit_pause', $dataupdate);           
        ...
        $this->db->delete('visit_pause',array('visitid' => $visitid, 'starttime' => $starttime,));
        $result = $this->Model_InjectDB->insertVisitPause($data);
        if($result){
          $this->response([
            'status' => TRUE,
            'message' =>'Insert Sukses',
...
public function injectVisit_post(){
 ...
 $year = $this->post('year');
 $status = $this->post('status');
 $checkintime = $this->post('checkintime');
 $checkouttime = $this->post('checkouttime');
...
//--- cek data
     ...
     if ($cekdata){
        ...
        $transacid = $cekdata[0]['id'];
        // -- delete visit pause
        $prosesdeldetail = $this->db->delete('visit_pause',array('visitid' => $transacid,));
        if ($prosesdeldetail){
 		$this->db->where('id', $transacid);
...
$this->response([
        ...
        $result = $this->Model_InjectDB->InjectVisit($userid,$customerno,$visitdate,$notvisitreason,$notbuyreason,$regionid,$salesofficeid,$salesgroupid,$salesdistrictid,$cycle,$week,$year,$this->udate('Y-m-d H:i:s.u T'),$status,$checkintime,$checkouttime);
        If($result){
          $this->response([
            'status' => TRUE,
            'message' =>'Insert Sukses',
